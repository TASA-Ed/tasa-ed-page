<script lang="ts">
  ...
  const desc =
    "TASA-Ed 工作室 官网，成立于 2020 年 12 月 20 日，一个专注于自媒体，软件游戏开发，网站搭建的工作室，而且，我们喜欢开源！";
  const title = "TASA-Ed 官网 - 404";
...
<div class="mx-auto flex items-center justify-center px-4 sm:px-6 lg:px-8 py-12 pb-20">
  <div class="max-w-2xl w-full text-center">
    <!-- 404 数字动画 -->
    <div class="mb-8 {mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}">
      <h1 class="text-9xl sm:text-[12rem] font-semibold leading-tight">404</h1>
    </div>
    ...
    >
      <h2 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mb-4">页面未找到</h2>
    ...
    <!-- 操作按钮 -->
    <div
      class="flex flex-col sm:flex-row gap-4 justify-center {mounted
        ? 'opacity-100 translate-y-0'
        : 'opacity-0 translate-y-4'} transition-all duration-700 delay-300 ease-out"
    >
      <button
        onclick={goHome}
        title="返回首页"
