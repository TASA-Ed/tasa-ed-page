...
<script lang="ts">
  ...
  import type { Comment } from '$lib';
	...
	const desc = "剧本编辑器留言簿的备份，由于 LeanCloud 将于 2027 年 1 月 12 日关停，所以我们备份了剧本编辑器的留言簿。";
...
<div class="kawaii-root min-h-screen px-4 py-10 font-sans">
  ...
  <header class="mb-10 text-center">
    <div class="inline-flex items-center gap-2.5 rounded-full border border-pink-200 bg-white/80 px-6 py-2.5 shadow-sm backdrop-blur dark:border-pink-500/30 dark:bg-slate-800/80">
      <span class="text-lg">💬</span>
      <h1 class="kawaii-title text-xl font-bold text-pink-500 dark:text-pink-400">剧本编辑器留言簿备份</h1>
      <span class="text-lg">✨</span>
    </div>
  ...
  {#if loading}
    ...
    <!-- ── Error ── -->
  {:else if error}
    <div class="mx-auto max-w-md rounded-3xl border border-pink-200 bg-white/80 p-6 text-center shadow dark:border-pink-500/20 dark:bg-slate-800/80 dark:shadow-pink-500/5">
  ...
  {:else if threads.length === 0}
    <div class="mx-auto max-w-md rounded-3xl border border-pink-200 bg-white/80 p-8 text-center shadow dark:border-pink-500/20 dark:bg-slate-800/80 dark:shadow-pink-500/5">
  ...
  {:else}
    <ul class="mx-auto flex max-w-2xl flex-col gap-5">
      {#each threads as thread (thread.root.objectId)}
        ...
        <li class="comment-card group rounded-3xl border border-white/80 bg-white/70 p-5 shadow-md backdrop-blur transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-700/60 dark:bg-slate-800/70 dark:shadow-slate-900/30 dark:hover:shadow-slate-900/50" id={thread.root.objectId}>

          <!-- Root comment -->
          <div class="flex gap-3">
            <!-- Avatar -->
            <div class="avatar-blob shrink-0 {pal.bg} {pal.text} flex h-10 w-10 items-center justify-center rounded-2xl text-sm font-bold shadow-sm dark:shadow-none">
          ...
          {#if thread.replies.length > 0}
            <ul class="mt-4 flex flex-col gap-3 border-l-2 border-dashed border-pink-100 pl-4 dark:border-pink-500/30">
              {#each thread.replies as reply (reply.objectId)}
                {@const rpal = avatarPalette(reply.objectId)}
                <li class="flex gap-3" id={reply.objectId}>
                  <!-- Reply avatar -->
                  <div class="avatar-blob shrink-0 {rpal.bg} {rpal.text} flex h-8 w-8 items-center justify-center rounded-xl text-xs font-bold shadow-sm dark:shadow-none">
