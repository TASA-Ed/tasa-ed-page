...
<section class="mx-auto flex max-w-6xl flex-col gap-6 px-6 pb-10 pt-16">
  <div class="space-y-4">
    ...
    <p class="max-w-2xl text-base text-slate-600 dark:text-slate-300">获取我们的项目和工具</p>
  </div>
</section>
  ...
  {#each groups as group (group.id)}
    <div class="space-y-4">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <h2 class="text-2xl font-semibold text-slate-900 dark:text-white md:text-3xl">
          {group.title}
        </h2>
      </div>
        ...
        {#each group.items as item (item.id)}
          ...
          >
            ...
            <div class="flex flex-col items-end gap-3 sm:flex-row sm:items-center">
              ...
              >
                <span
                  class="flex gap-1 rounded-full border border-slate-200/80 px-3 py-1 dark:border-slate-700/80"
                  ><Calendar
